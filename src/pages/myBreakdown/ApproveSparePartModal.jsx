...
const ApproveSparePartModal = ({ open, onClose, data, onSubmit }) => {
  ...
  const onAssignUser = async () => {
    const payload = {
      ...
      breakdownSpareRequestId: data._id || data?.id,
  ...
  const onFinishSave = async () => {
    ...
    if (data?.requestStatus === breakdownSpareRequestStatus.approved) {
      ...
      if (res?.code === 1) {
        ...
        await getAllBreakdownSpareRequestBySpareRequestId();

        if (assignUserIds && assignUserIds.length > 0) {
          if (!values.sendType) {
            message.error(
              t("breakdown.spareRequest.modal.validation.send_type_required")
            );
            return;
          }
          if (!values.comment) {
            message.error(
              t("breakdown.spareRequest.modal.validation.comment_required")
            );
            return;
          }
          onAssignUser();
  ...
  const showButtonRejeced = (item) =>
    item?.requestStatus === breakdownSpareRequestDetailStatus.approved;

  const showButtonRecovered = (item) =>
    item?.requestStatus === breakdownSpareRequestDetailStatus.rejected &&
    data?.requestStatus !== breakdownSpareRequestStatus.submitted;
    ...
    >
      <Form layout="vertical" form={form}>
        ...
        <Row gutter={[24, 24]} className="p-2">
          ...
          <div className="p-3">
            {data?.requestStatus === breakdownSpareRequestStatus.approved &&
              !isAllRejected && (
                <Row>
                  <Col span={6}>
                    <strong>
